...
class CompleteThreeYearAnalyzer:
    """
    Complete Three-Year Analysis with Golden Theme
    ...
    def _load_all_data(self):
        ...
        try:
            ...
            # Extract ANON MRN and Last mRS from each year
            mrs_2020_clean = mrs_2020[['ANON MRN ', 'Last mRS']].dropna()
            mrs_2021_clean = mrs_2021[['ANON MRN ', 'Last mRS']].dropna()
            mrs_2022_clean = mrs_2022[['ANON MRN ', 'Last mRS']].dropna()
            
            # Add year column
            mrs_2020_clean['Year'] = 2020
            mrs_2021_clean['Year'] = 2021
            mrs_2022_clean['Year'] = 2022
    ...
    def _aggregate_by_patient(self):
        ...
        if self.mrs_combined is not None:
            # Create a mapping from PatientID to mRS outcomes
            patient_mrs_mapping = {}
            
            # Map ANON MRN to PatientID format and get mRS
            for _, row in self.mrs_combined.iterrows():
                mrn_anon = row['ANON MRN ']
                last_mrs = row['Last mRS']
                year = row['Year']
                
                # Convert ANON MRN to PatientID format (DE-IDENTIFIED, XXXX.brainlab)
                patient_id = f"DE-IDENTIFIED, {mrn_anon}.brainlab"
    ...
    def create_analysis(self, target_col='Last mRS', output_path='complete_three_year_analysis.pdf'):
        """Create complete three-year analysis with golden theme"""
        ...
        with PdfPages(output_path) as pdf:
            ...
            # 1. COMPLETE THREE-YEAR DATA OVERVIEW
            self._create_complete_overview(pdf, target_col)
        ...
        print(f"✅ Complete three-year analysis saved to {output_path}")
    
    def _create_complete_overview(self, pdf, target_col):
        """Create complete three-year data overview with golden theme"""
        ...
        fig.suptitle('Complete Three-Year Patient-Level Data Overview', fontsize=16, fontweight='bold', color=GOLDEN_COLORS['dark_gold'])
        ...
        table.scale(1, 2)
        axes[1, 1].set_title('Complete Dataset Summary', fontweight='bold', color=GOLDEN_COLORS['dark_gold'])
        axes[1, 1].axis('off')
        ...
        print(f"Complete Three-Year Patient-Level Data Overview:")
    ...
    def _create_model_comparison(self, pdf, target_col):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Complete Three-Year Patient-Level Model Performance: Train vs Test', fontsize=16, fontweight='bold', color=GOLDEN_COLORS['dark_gold'])
        ...
        # Print results
        print(f"\nComplete Three-Year Patient-Level Model Performance Summary:")
    ...
    def _create_feature_importance(self, pdf, target_col):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Complete Three-Year Patient-Level Feature Importance Analysis', fontsize=16, fontweight='bold', color=GOLDEN_COLORS['dark_gold'])
    ...
    def _create_feature_set_analysis(self, pdf, target_col):
        ...
        fig.suptitle('Complete Three-Year Patient-Level Prediction Accuracy by Feature Set', fontsize=16, fontweight='bold', color=GOLDEN_COLORS['dark_gold'])
        ...
        # Print results
        print(f"\nComplete Three-Year Patient-Level Feature Set Analysis:")
        for name, result in results.items():
    ...
    def _create_cv_analysis(self, pdf, target_col):
        ...
        # Create visualization with golden theme
        fig, axes = plt.subplots(1, 2, figsize=(16, 6))
        fig.suptitle('Complete Three-Year Patient-Level Cross-Validation Analysis', fontsize=16, fontweight='bold', color=GOLDEN_COLORS['dark_gold'])
        ...
        # Print results
        print(f"\nComplete Three-Year Patient-Level Cross-Validation Results:")
        for name, result in cv_results.items():
            print(f"{name}: {result['mean']:.3f} ± {result['std']:.3f}")

def main():
    """Main function to run complete three-year analysis"""
    print("=== COMPLETE THREE-YEAR PATIENT-LEVEL GOLDEN RADIOMICS ANALYSIS ===")
    ...
    analyzer = CompleteThreeYearAnalyzer()
    
    # Create analysis
    analyzer.create_analysis('Last mRS', 'complete_three_year_analysis.pdf')
    
    print("\n=== ANALYSIS COMPLETED ===")
    print("Generated analyses include:")
    print("1. Complete Three-Year Data Overview")
    ...
    print("- Complete three-year dataset with mRS outcomes")
